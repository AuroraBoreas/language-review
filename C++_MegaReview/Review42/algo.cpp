...
void TS::algo_set(void)
{
    ...
    v.clear();
    std::merge(A.begin(), A.end(), B.begin(), B.end(), std::inserter(v, v.end()));
    TS::print("std::merge, v-> ", v);

    v.clear();
    std::set_difference(A.begin(), A.end(), B.begin(), B.end(), std::inserter(v, v.end()));
    TS::print("std::set_difference, v-> ", v);

    v.clear();
    std::set_intersection(A.begin(), A.end(), B.begin(), B.end(), std::inserter(v, v.end()));
    TS::print("std::set_intersection, v-> ", v);

    v.clear();
    std::set_symmetric_difference(A.begin(), A.end(), B.begin(), B.end(), std::inserter(v, v.end()));
...
void TS::algo_query_value(void)
{
    ...
    TS::oformat("std::adjacent_difference, rv-> ");
...
void TS::algo_query_property(void)
{
    ...
    auto [mi, ma] = std::minmax_element(v.begin(), v.end(), std::less<>{});
    ...
    rv = std::equal(m.begin(), m.end(), n.begin(), n.end());
...
void TS::algo_lone_island(void)
{
    ...
    std::transform(m.begin(), m.end(), std::ostream_iterator<char>(std::cout, " "), [](const char& e){ return std::toupper(e); });
    TS::newline();
}
