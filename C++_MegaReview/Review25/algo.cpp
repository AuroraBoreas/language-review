...
void TS::algo_mover(void)
{
    ...
    TS::oformat("std::move, t-> \n");
    std::move(vt.begin(), vt.end(), std::back_inserter(lt));
    for(auto& t : lt) t.join();

    using scontainer = std::vector<std::string>;
    scontainer m{"hello", "world"};
    scontainer n{"zhang", "liang"};
    TS::print("std::copy_backward, b4, m-> ", m);
    TS::print("std::copy_backward, b4, n-> ", n);

    std::copy_backward(m.begin(), m.end(), n.end());
    TS::print("std::copy_backward, af, m-> ", m);
    TS::print("std::copy_backward, af, n-> ", n);

    m = {"hello", "world"};
    n = {"zhang", "liang"};
    std::move_backward(m.begin(), m.end(), n.end());
    TS::print("std::move_backward, af, m-> ", m);
    TS::print("std::move_backward, af, n-> ", n);
...
void TS::algo_set(void)
{
    ...
    S.clear();
    std::merge(A.begin(), A.end(), B.begin(), B.end(), std::inserter(S, S.begin()));
...
void TS::algo_query_value(void)
{
    ...
    TS::oformat("std::count, cnt-> "); std::cout << cnt << std::endl;

    double rv = std::accumulate(v.begin(), v.end(), 0.0);
    TS::oformat("std::accumulate, rv-> "); std::cout << rv << std::endl;
    ...
    TS::oformat("std::inner_product, rv-> "); std::cout << rv << std::endl;
...
void TS::algo_lone_island(void)
{
    ...
    std::string n;
    std::transform(m.begin(), m.end(), std::back_inserter(n), [](const auto& e){ return std::toupper(e); });
