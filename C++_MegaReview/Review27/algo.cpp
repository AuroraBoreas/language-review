...
void TS::algo_set(void)
{
    ...
    bool rv = std::includes(A.begin(), A.end(), B.begin(), B.end(), std::back_inserter(S));
    TS::oformat("std::includes, S-> "); std::cout << std::boolalpha << rv << std::endl;
}
...
void TS::algo_query_property(void)
{
    ...
    rv = std::binary_search(v.begin(), v.end(), 42);
