...
void TS::aglo_permutation_heap(void)
{
    std::vector<int> v{2,1,3,99,99,99,0,-1,88};
    ...
    TS::print("std::next_permuation, v-> ", v);
}
...
void TS::algo_permutation_sort(void)
{
    ...
    /// psr
    std::partition(v.begin(), v.end(), [](int const& e)->bool{ return e%2==0; });
    TS::print("std::partition, v-> ", v);

    auto g = [](const int& e){ return e%2==1; };
    auto it = std::partition_point(v.begin(), v.end(), g);
    TS::oformat("std::partition_point, v-> ");
    (it != std::end(v))
...
void TS::algo_query_property(void)
{
    ...
    TS::oformat("std::accumulate, n-> ");

    std::vector<int> u{2,1,3,99,99,99,0,-1,88};
...
void TS::algo_raw_memory(void)
{
     ...
     * uninitializeed_value_construct
     * uninitialized_fill
     * uninitialzied_move
     * uninitialized_copy
    */
